...
const LIVE_ORIGIN = 'https://ai-trader.best'
const LIVE_HOST = 'ai-trader.best'
const ALT_HOSTS = new Set(['www.ai-trader.best'])
const ANNUAL_DISCOUNT_MULTIPLIER = 0.5
const INDEXNOW_KEY = '3b78b01a4a3a4e0d9f06f6f1f22a5b71'
...
const planCatalog = {
  starter: {
    ...
    currency: 'USD',
    summary: 'single-agent AI trading signal evaluation',
  },
  pro: {
    id: 'pro',
    name: 'Operator',
    ...
    currency: 'USD',
    summary: 'default supervised AI-Trader operator desk',
  },
  desk: {
    id: 'desk',
    name: 'Desk',
    ...
    currency: 'USD',
    summary: 'multi-agent publishing, review, and competition governance',
  },
}
...
function resolveConfiguredProductId(env, planId, billing) {
  ...
  const keys = [
    ...
    `POLAR_PRODUCT_ID_AI_TRADER_${normalizedSelection}`,
...
async function getOrCreatePolarProduct(env, apiKey, plan, billing, successUrl) {
  ...
  const product = await requestPolarJson(apiKey, `${resolvePolarBase(env)}/v1/products`, {
    name: `AI-Trader ${plan.name} (${billingLabel})`,
...
async function handleCheckout(request, env, requestUrl) {
  ...
  try {
    ...
    const checkout = await requestPolarJson(apiKey, `${resolvePolarBase(env)}/v1/checkouts`, {
      ...
      request_id: `ai_trader_${Date.now()}_${Math.random().toString(16).slice(2)}`,
      metadata: {
        site: 'ai-trader.best',
        planId: plan.id,
        billing,
...
async function handleAnalytics(request, env) {
  return handleAnalyticsRequest(request, env, { siteKey: 'ai-trader' })
}
