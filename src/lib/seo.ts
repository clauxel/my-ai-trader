...
const liveOrigin = 'https://ai-trader.best'
...
const canonicalLinkId = 'ai-trader-canonical-link'
const structuredDataScriptId = 'ai-trader-structured-data'
...
export function buildSeoDocument(args: {
  ...
  if (routeView === 'pricing') {
    const title = 'AI-Trader Pricing | Operator Annual Trading Agent Desk'
    const description =
      'Compare AI-Trader Starter, Operator, and Desk plans. Operator annual is selected by default and annual billing is 50% cheaper than monthly.'
    return {
      title,
      description,
      canonicalUrl,
  ...
  if (routeView === 'privacy') {
    ...
    const description = 'How AI-Trader handles analytics, checkout metadata, and public site interactions.'
    return {
      title,
      description,
      canonicalUrl,
  ...
  if (routeView === 'terms') {
    ...
    const description = 'Terms for using the AI-Trader site, hosted payment flow, and supervised trading-agent workflow.'
    return {
      title,
      description,
      canonicalUrl,
  ...
  return {
    ...
    description: 'The requested AI-Trader page was not found.',
    canonicalUrl,
    robots: 'noindex,nofollow',
